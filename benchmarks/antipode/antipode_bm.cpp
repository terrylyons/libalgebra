...
//    const void* src = input_tensor.base_vector().as_ptr();
    ...
    long unsigned int N = std::max((4500*1024)/(sizeof(float)*input_tensor.base_vector().dimension()),1ul);

    std::vector<TENSOR> src = {N, input_tensor};
//    std::cout << "size of src: " << N*sizeof(float)*input_tensor.base_vector().dimension() << std::endl;
    std::vector<TENSOR> dest(N, result);
    ...
    for (auto _: state) {
        ...
        for (int i=0; i<src.size(); i++) {
            ...
            std::memcpy(dest[i].base_vector().as_mut_ptr(), src[i].base_vector().as_ptr(), sizeof(float)*input_tensor.base_vector().dimension());
        }

        benchmark::ClobberMemory();
    }

    state.SetBytesProcessed(N*sizeof(float)*state.iterations()*input_tensor.base_vector().dimension());
...
//    }
//
//    state.SetBytesProcessed(sizeof(float)*state.iterations()*input_tensor.base_vector().dimension());
//
//}
...
static void BM_antipode(benchmark::State& state) {
    ...
    for (auto _: state) {
        ...
        //        result = antipode(input_tensor);
        for (int i=0; i<src.size(); i++) {
            dtl::tiled_inverse_operator<Width, Depth, float_field, dtl::default_signer, TileLetters>::apply(src[i].base_vector(), dest[i].base_vector());
            benchmark::ClobberMemory();
        }
    }
