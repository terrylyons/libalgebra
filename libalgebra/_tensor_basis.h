...
struct NoWords {
    enum {
        ans = No_Letters * NoWords<No_Letters,
                                   DEPTH - 1>::ans + 1
    };
};

template <unsigned No_Letters>
struct NoWords<No_Letters,
               0> {
    enum {
        ans = 1
    };
...
public:
    ...
    ///Checks that the DEPTH does not exceed the Maximum word length.
    _tensor_basis(_tensor_basis<No_Letters2,
                                DEPTH2> arg, translator h)
            : _word((word_t) 1.) {
        STATIC_ASSERT(DEPTH <= uMaxWordLength);
        //static_assert(DEPTH <= uMaxWordLength, "specified length of words in tensor basis exceeds size available ");
    ...
    inline unsigned size() const {
        fp_info<word_t>::unsigned_int_type sz = (fp_info<word_t>::signed_int_type(
                (reinterpret_cast<const fp_info<word_t>::unsigned_int_type &>(_word) & fp_info<word_t>::exponent_mask)
                        >> fp_info<word_t>::mantissa_bits_stored) - fp_info<word_t>::exponent_bias) / uBitsInLetter;
#ifdef _DEBUG
        int iExponent;
    ...
    friend std::ostream &operator<<(std::ostream &os, const _tensor_basis<No_Letters,
                                                                          DEPTH> &word) {
        int iExponent;
