...
/* The algebra is a vector with an additional multiplication operation which is "compatible" with the
 * addition and scalar multiplication. We attempt to mimic this with the algebra template class below.
 * For this, we need to define "multiplication" types for each of the algebras that we wish to
 * implement. Here is a simple example of what a multiplication class should look like.
 *
 ...
 *          key_transform<scalar_t, Operator> kt(op);
 ...
 *          typedef typename Algebra::SCALAR scalar_t;
 *          Algebra result;
...
template <typename Basis,
        ...
        typename VectorType>
class algebra : public vectors::vector<Basis,
                                       Coeff,
                                       VectorType> {
...
public:
    ...
    template <typename Transform>
    inline void buffered_apply_binary_transform(algebra &result, const algebra &rhs, Transform fn,
                                                one_method_multiplication_tag) const {
        typename basis_multiplication_selector<BASIS>::template key_operator<BASIS,
                                                                             Coeff,
                                                                             Transform> key_fn(fn);
    ...
    template <typename Transform>
    inline void buffered_apply_binary_transform(algebra &result, const algebra &rhs, Transform fn,
                                                two_method_multiplication_tag) const {
        typename basis_multiplication_selector<BASIS>::template index_operator<BASIS,
                                                                               Coeff,
                                                                               Transform> index_fn(fn);
        typename basis_multiplication_selector<BASIS>::template key_operator<BASIS,
                                                                             Coeff,
                                                                             Transform> key_fn(fn);
    ...
    template <typename Transform>
    inline void unbuffered_apply_binary_transform(const algebra &rhs, Transform fn, one_method_multiplication_tag) {
        typename basis_multiplication_selector<BASIS>::template key_operator<BASIS,
                                                                             Coeff,
                                                                             Transform> key_fn(fn);
    ...
    template <typename Transform>
    inline void unbuffered_apply_binary_transform(const algebra &rhs, Transform fn, two_method_multiplication_tag) {
        typename basis_multiplication_selector<BASIS>::template index_operator<BASIS,
                                                                               Coeff,
                                                                               Transform> index_fn(fn);
        typename basis_multiplication_selector<BASIS>::template key_operator<BASIS,
...
public:
    ...
    inline friend algebra commutator(const algebra &a, const algebra &b) { // Returns a * b - b * a
        algebra result;
