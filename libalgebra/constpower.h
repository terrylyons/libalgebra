...
struct ConstPower {
    enum : long long {
        ...
        ans = ConstPower<arg,
                         exp / 2>::ans * ConstPower<arg,
                                                    exp / 2>::ans * ((exp % 2 == 0) ? 1ULL : arg),

    };
};

template <unsigned long long arg>
struct ConstPower<arg,
                  0> {
    enum : long long {
...
struct TestConstPower {
    enum {
        intermediate = ConstPower<arg,
                                  exp>::ans * TestConstPower<arg,
                                                             exp - 1>::intermediate,
        ans = (intermediate == ConstPower<arg,
                                          (ConstPower<exp,
                                                      2>::ans + exp) / 2>::ans) && TestConstPower<arg - 1,
                                                                                                  exp>::ans,
        enum1_force_long_long = 9223372036854775807ULL
    };
};
...
template <unsigned long long arg>
struct TestConstPower<arg,
                      0> {
    enum {
        intermediate = ConstPower<arg,
                                  0>::ans, ans = (intermediate == 1ULL), enum1_force_long_long = 9223372036854775807ULL
    };
};

template <alg::DEG exp>
struct TestConstPower<1,
                      exp> {
    enum {
        intermediate = ConstPower<1,
                                  exp>::ans, ans = (intermediate == 1)
    };
};
