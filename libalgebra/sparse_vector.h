...
//  unordered_sparse_vector is not ordered; it is significatly faster for normal
//  functions however iterators may be invalidated by any sort of insertion in
//  the unordered settings
/**
...
public:
  ...
  inline sparse_vector &operator+=(const sparse_vector &rhs) {
    ...
    if (empty())
      return *this = rhs;
    for (cit = rhs.begin(); cit != rhs.end();
         ++cit) { // Instead of a bare (*this)[cit->first] += cit->second;
      it = find(cit->first);
      if (it == end())
...
// these min max operators are slower (factor of 3?) on unordered sparse vectors
#ifdef UNORDEREDMAP
    {
      typename std::vector<std::pair<KEY, SCALAR>> target(begin(), end()),
          source(rhs.begin(), rhs.end());
      const auto &comp = [](typename std::pair<KEY, SCALAR> lhs,
                            typename std::pair<KEY, SCALAR> rhs) -> bool {
        return lhs.first < rhs.first;
      };
      std::sort(target.begin(), target.end(), comp);
      std::sort(source.begin(), source.end(), comp);
      typename std::vector<std::pair<KEY, SCALAR>>::iterator it =
          target.begin();
  ...
  inline sparse_vector &add_scal_prod(const sparse_vector &rhs,
    ...
    const_iterator cit;
    for (cit = rhs.begin(); cit != rhs.end();
         ++cit) { // Instead of a bare (*this)[cit->first] += cit->second * s;
      it = this->insert(
          it, std::make_pair(cit->first,
                             zero)); // note this fails if the entry is already
...
template <class BASIS, class MAP> BASIS sparse_vector<BASIS, MAP>::basis;
