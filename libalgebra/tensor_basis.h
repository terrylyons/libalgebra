...
struct depth_size {
    enum {
        value = (ConstPower<NoLetters,
                            Depth>::ans - 1) / (NoLetters - 1)
    };
};
/*
...
struct tensor_size_info {
    ...
    static const LIBALGEBRA_STATIC_ARRAY_TYPE<DIMN,
                                              max_depth + 2> degree_sizes;

};

template <DEG NoLetters>
LIBALGEBRA_STATIC_ARRAY_TYPE<DIMN,
                             tensor_size_info<NoLetters>::max_depth + 2> populate_tensor_size_info_array() {
    LIBALGEBRA_STATIC_ARRAY_TYPE<DIMN,
                                 tensor_size_info<NoLetters>::max_depth + 2> tmp;
    alg::utils::populate_array<depth_size,
                               NoLetters,
                               tensor_size_info<NoLetters>::max_depth + 1>::fill(tmp);
    return tmp;
}

template <DEG NoLetters> const LIBALGEBRA_STATIC_ARRAY_TYPE<DIMN,
                                                            tensor_size_info<NoLetters>::max_depth + 2>
...
public:
    ...
    typedef _tensor_basis<n_letters,
                          max_degree> KEY;
...
public:
    ...
    tensor_basis(void) : _size((LET(ConstPower<n_letters,
                                               max_degree + 1>::ans) - 1) / (n_letters - 1)) {}
...
public:
    ...
    static KEY index_to_key(const DIMN idx) {
        ...
        typename std::map<DIMN,
                          KEY>::iterator it = __cache.find(idx);
        if (it != __cache.end()) {
            return it->second;
        }
...
*/
template <DEG n_letters, DEG max_degree>
class free_tensor_basis : public tensor_basis<n_letters,
                                              max_degree>,
...
public:
    ...
    typedef tensor_basis<n_letters,
                         max_degree> TBASIS;
...
//}
    ...
    inline friend std::ostream &operator<<(std::ostream &os, const std::pair<free_tensor_basis *,
                                                                             KEY &> &t) {
...
struct vector_type_selector<free_tensor_basis<n_letters,
                                              max_depth>,
                            Field> {
    typedef free_tensor_basis<n_letters,
                              max_depth> BASIS;
    typedef typename BASIS::KEY KEY;
    typedef vectors::sparse_vector<BASIS,
                                   Field,
                                   #ifndef ORDEREDMAP
                                   MY_UNORDERED_MAP<KEY,
                                                    typename Field::S,
                                                    typename KEY::hash>
#else
    ...
    typedef vectors::dense_vector<BASIS,
                                  Field,
                                  std::vector<typename Field::S> > dense_vect;

    typedef vectors::hybrid_vector<BASIS,
                                   Field,
    ...
    typedef typename alg::utils::type_selector<boost::is_pod<typename Field::S>::value,
                                               sparse_vect,
...
struct basis_multiplication_selector<free_tensor_basis<n_letters,
                                                       max_depth> > {
...
*/
template <DEG n_letters, DEG max_degree>
class shuffle_tensor_basis : public tensor_basis<n_letters,
                                                 max_degree>,
                             public basis_traits<With_Degree,
                                                 n_letters,
                                                 max_degree> {
...
public:
    ...
    */
    inline const TENSOR &prod(const KEY &k1, const KEY &k2) const {
        ...
        typedef std::map<std::pair<KEY,
                                   KEY>,
                         TENSOR> TABLE_T;
        static TABLE_T table;
        typename TABLE_T::iterator it;
        std::pair<KEY,
                  KEY> p(std::min(k1, k2), std::max(k1, k2));
        it = table.find(p);
        if (it == table.end()) {
    ...
    inline friend std::ostream &operator<<(std::ostream &os, const std::pair<shuffle_tensor_basis *,
                                                                             KEY> &t) {
        return os << (t.first)->key2string(t.second);
    }
    ...
    inline void tensor_add_mul(const TENSOR &a, const TENSOR &b, TENSOR &ans) const {
        ...
        for (i = a.begin(); i != a.end(); ++i) {
            for (j = b.begin(); j != b.end(); ++j) {
                ans.add_scal_prod(free_tensor_prod(i->first, j->first), i->second * j->second);
            }
        }
    }
    /// The concatenation product of two basis elements.
...
template <DEG n_letters, DEG max_depth, typename Field>
struct vector_type_selector<shuffle_tensor_basis<n_letters,
                                                 max_depth>,
                            Field> {
    typedef shuffle_tensor_basis<typename Field::S,
                                 typename Field::Q,
                                 n_letters,
                                 max_depth> BASIS;
    typedef typename BASIS::KEY KEY;
