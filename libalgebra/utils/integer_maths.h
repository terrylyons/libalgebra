...
struct divisor_calc {
    typedef typename divisor_calc<N,
                                  D + 1>::next next;
    enum {
        num = N,
        divisor = divisor_calc<N,
                               D + 1>::divisor,
        quotient = divisor_calc<N,
                                D + 1>::quotient
    };
...
struct divisor_calc<N,
                    D,
                    Q,
                    0> {
    typedef divisor_calc<Q> next;
    enum {
        num = N, divisor = D, quotient = Q
    };
...
struct divisor_calc<1,
                    D,
                    Q,
                    R> {
    typedef divisor_calc<1> next;
    enum {
        num = 1, divisor = 1, quotient = 1
    };
};
...
struct square_free {
    static const bool ans = square_free<typename Divisor::next,
...
struct square_free<divisor_calc<1>,
                   false,
                   LastNum> {
...
struct mobius_func_impl {
    static const Long value = -1 * mobius_func_impl<typename Divisor::next,
                                                    (Divisor::num > 1),
                                                    true>::value;
};
...
struct mobius_func_impl<Divisor,
                        B,
                        false> {
...
*/
template <bool B>
struct mobius_func_impl<divisor_calc<1>,
                        B,
                        true> {
