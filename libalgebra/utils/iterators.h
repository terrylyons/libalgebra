...
template <typename Vector, typename Iterator> inline typename boost::enable_if<typename boost::mpl::and_<
        boost::is_same<typename boost::remove_cv<typename std::iterator_traits<Iterator>::value_type::first_type>::type,
                       typename Vector::KEY>, boost::is_same<
                typename boost::remove_cv<typename std::iterator_traits<Iterator>::value_type::second_type>::type,
                typename Vector::SCALAR>>::type, typename std::iterator_traits<Iterator>::value_type::first_type>::type
key(Iterator &it)
{
    return it->first;
}
...
template <typename Vector, typename Iterator, typename ValueType = typename std::iterator_traits<Iterator>::value_type,
          typename Reference = typename std::iterator_traits<Iterator>::reference> inline typename boost::enable_if<
        ...
        typename copy_constness<Reference, typename ValueType::second_type>::type &>::type value(Iterator &it)
{
    return it->second;
}
