...
class dense_vector : protected base_vector<Basis,
                                           Coeffs>, dtl::requires_order<Basis> {
...
public:
    ...
    void insert(InputIterator begin, InputIterator end) {
        typedef std::vector<std::pair<DIMN,
                                      SCALAR> > TMPVEC;
        TMPVEC tmp;
        ...
        for (InputIterator it = begin; it != end; ++it) {
            ...
            tmp.push_back(std::pair<DIMN,
                                    SCALAR>(curr, it->second));
        }
...
protected:
    ...
    std::pair<DIMN,
              bool> equal_to_min(const dense_vector &rhs) const {
        DIMN mid = std::min(m_dimension, rhs.m_dimension);

        for (DIMN i = 0; i < mid; ++i) {
            if (m_data[i] != rhs.m_data[i]) {
                return std::pair<DIMN,
                                 bool>(mid, false);
            }
        }
        return std::pair<DIMN,
...
public:
    ...
    void triangular_buffered_apply_binary_transform(Vector &result, const dense_vector &rhs, KeyTransform key_transform,
    ...
    void triangular_unbuffered_apply_binary_transform(const dense_vector &rhs, KeyTransform key_transform,
                                                      const DEG max_depth) {
        dense_vector result;
    ...
    template <typename Vector,
            typename KeyTransform,
            typename IndexTransform>
    void square_buffered_apply_binary_transform(Vector &result, const dense_vector &rhs, KeyTransform /*key_transform*/,
        ...
        index_transform(&d_result.m_data[0], &m_data[0], &rhs.m_data[0], m_dimension, rhs.m_dimension);
    }
