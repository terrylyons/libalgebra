...
template <typename Basis,
        typename Coeffs,
        typename ResizePolicy=policy::basic_resize_policy,
        typename DenseStorage=std::vector<typename Coeffs::S>,
        typename SparseMap=LIBALGEBRA_DEFAULT_MAP_TYPE >
class hybrid_vector : public dense_vector<Basis,
                                          Coeffs,
                                          DenseStorage>,
                      public sparse_vector<Basis,
                                           Coeffs,
                                           SparseMap> {
...
private:
    ...
    static bool sort_by_index(std::pair<DIMN,
                                        SCALAR> p1, std::pair<DIMN,
                                                              SCALAR> p2) {
    ...
    void incorporate_sparse() {
        ...
        while (it != end) {
            if ((idx = key_to_index(it->key())) < dense_dim) {
                buffer.push_back(std::pair<DIMN,
                                           SCALAR>(idx, it->value()));
        ...
        typename std::vector<std::pair<DIMN,
                                       SCALAR> >::const_iterator cit;
...
public:
    ...
    public:
        ...
        iterator_item(hybrid_vector &vect, typename DENSE::iterator it) : m_dense_iterator(it),
                                                                          m_dense_end(vect.dense_part().end()),
                                                                          m_sparse_begin(vect.sparse_part().begin()),
                                                                          m_sparse_iterator(
    ...
    public:
        ...
        const_iterator_item(const hybrid_vector &vect, typename SPARSE::const_iterator it) : m_dense_iterator(
                ...
                vect.sparse_part().begin()), m_sparse_iterator(it) {}

        const_iterator_item(const hybrid_vector &vect, typename DENSE::const_iterator it) : m_dense_iterator(it),
                                                                                            m_dense_end(
                                                                                                    vect.dense_part()
                                                                                                        .end()),
                                                                                            m_sparse_begin(
    ...
    iterator insert(iterator it, const std::pair<const KEY,
                                                 SCALAR> &val) {
        std::pair<iterator,
                  bool> rv = insert(val);
        return rv.first;
    ...
    std::pair<iterator,
              bool> insert(const KEY key, SCALAR val) {
        std::pair<iterator,
                  bool> rv(end(), false);
        DIMN idx;
        if ((idx = key_to_index(key)) < dense_dimension()) {
            ...
            std::pair<const KEY,
                      SCALAR> tmp_pair(key, val);
            std::pair<typename SPARSE::iterator,
                      bool> tmp = SPARSE::insert(tmp_pair);
    ...
    std::pair<iterator,
              bool> insert(const std::pair<const KEY,
                                           SCALAR> &p) {
        DIMN idx;
        if ((idx = key_to_index(p.first)) < dense_dimension()) {
            if (DENSE::value(idx) == zero && p.second != zero) {
                ...
                return std::pair<iterator,
                                 bool>(iterator(*this, DENSE::find_index(idx)), true);
            }
        } else {
            typename SPARSE::iterator it = SPARSE::find(p.first);
            if (p.second != zero) {
                std::pair<typename SPARSE::iterator,
                          bool> ins = SPARSE::insert(p);
                return std::pair<iterator,
                                 bool>(iterator(*this, ins.first), ins.second);
            }
        }
        return std::pair<iterator,
                         bool>(end(), false);
...
public:
    ...
    bool operator==(const hybrid_vector &rhs) const {
        ...
        std::pair<DIMN,
                  bool> dense_part_eq = DENSE::equal_to_min(rhs);
...
public:
    ...
    static bool comp(std::pair<KEY,
                               SCALAR> lhs, std::pair<KEY,
                                                      SCALAR> rhs) {
        key_ordering ord;
...
private:
    ...
    void triangular_binary_transform_mixed_cases(Vector &result, const hybrid_vector &rhs, KeyTransform key_transform,
        ...
        std::vector<std::pair<KEY,
                              SCALAR> > buffer;
        std::vector<typename std::vector<std::pair<KEY,
                                                   SCALAR> >::const_iterator> iterators;
        SPARSE::separate_by_degree(buffer, rhs, max_depth, iterators);

        // Do the dense * sparse first
        typename std::vector<std::pair<KEY,
                                       SCALAR> >::const_iterator cit, buf_begin(buffer.begin());
    ...
    template <typename Vector,
            typename KeyTransform>
    void
    square_binary_transform_mixed_case(Vector &result, const hybrid_vector &rhs, KeyTransform key_transform) const {
        /*
         * At this stage there are three remaining cases:
         *   - dense * sparse;
         *   - sparse * dense;
        ...
        typename std::vector<std::pair<KEY,
                                       SCALAR> >::const_iterator cit, buf_begin(buffer.begin()), buf_end(buffer.end());
...
public:
    ...
    template <typename KeyTransform,
            typename IndexTransform>
    void triangular_unbuffered_apply_binary_transform(const hybrid_vector &rhs, KeyTransform key_transform,
    ...
    template <typename Vector,
            typename KeyTransform,
            typename IndexTransform>
    void square_buffered_apply_binary_transform(Vector &result, const hybrid_vector &rhs, KeyTransform key_transform,
