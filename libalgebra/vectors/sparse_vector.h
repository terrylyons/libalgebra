...
class sparse_vector
        : /*private*/ MapType,
                      protected base_vector<Basis,
                                            Coeffs> {
    typedef MapType MAP;
    typedef Basis BASIS;
    typedef base_vector<Basis,
                        Coeffs> BASE_VEC;
public:
...
public:
    ...
    std::pair<iterator,
        ...
        if (zero == value.second) {
            return std::pair<iterator,
                             bool>(iterator(*this, MAP::find(value.first)), false);
        }
        std::pair<typename MAP::iterator,
                  bool> p = MAP::insert(value);
        return std::pair<iterator,
                         bool>(iterator(*this, p.first), p.second);
    }

    // Redefine the other inserts
    std::pair<iterator,
              bool> insert(std::pair<const KEY,
                                     SCALAR> &value) {
        if (zero == value.second) {
            return std::pair<iterator,
                             bool>(iterator(*this, MAP::find(value.first)), false);
    ...
    iterator insert(iterator position, const std::pair<const KEY,
                                                       SCALAR> &value) {
...
// these min max operators are slower (factor of 3?) on unordered sparse vectors
#ifdef UNORDEREDMAP
        {
            typename std::vector<std::pair<KEY,
                                           SCALAR> > target(map_begin(), map_end()), source(rhs.map_begin(),
                                                                                            rhs.map_end());
            const auto &comp = [](typename std::pair<KEY,
                                                     SCALAR> lhs, typename std::pair<KEY,
                                                                                     SCALAR> rhs) -> bool {
            ...
            typename std::vector<std::pair<KEY,
                                           SCALAR> >::iterator it = target.begin();
            typename std::vector<std::pair<KEY,
                                           SCALAR> >::const_iterator cit = source.begin();
    ...
    inline sparse_vector &operator|=(const sparse_vector &rhs) {
        ...
        typename std::vector<std::pair<KEY,
                                       SCALAR> > target(map_begin(), map_end()), source(rhs.map_begin(), rhs.map_end());
        std::sort(target.begin(), target.end(), comp);
        std::sort(source.begin(), source.end(), comp);
    ...
    */
    ...
    static bool comp(typename std::pair<KEY,
                                        SCALAR> lhs, typename std::pair<KEY,
                                                                        SCALAR> rhs) {
        return lhs.first < rhs.first;
    };
...
protected:
    ...
    void print_members(std::ostream &os) const {
        ...
        typename std::vector<std::pair<KEY,
                                       SCALAR> > buffer(map_begin(), map_end());
...
protected:
    ...
    void fill_buffer(std::vector<std::pair<KEY,
                                           SCALAR> > &buffer) const {
    ...
    void separate_by_degree(std::vector<std::pair<KEY,
                                                  SCALAR> > &buffer, const sparse_vector &rhs, const size_t DEPTH1,
                            std::vector<typename std::vector<std::pair<KEY,
                                                                       SCALAR> >::const_iterator> &iterators) const {
        rhs.fill_buffer(buffer);
#ifndef ORDEREDMAP
        std::sort(buffer.begin(), buffer.end(), [](const std::pair<KEY,
                                                                   SCALAR> &lhs, const std::pair<KEY,
                                                                                                 SCALAR> &rhs) -> bool {
            return lhs.first < rhs.first;
        });
#endif // ORDEREDMAP
        ...
        unsigned deg = 0;
        for (typename std::vector<std::pair<KEY,
                                            SCALAR> >::const_iterator j0 = buffer.begin(); j0 != buffer.end(); j0++) {
...
public:
    ...
    triangular_buffered_apply_binary_transform(Vector &result, const sparse_vector &rhs, KeyTransform key_transform,
        ...
        typename std::vector<std::pair<KEY,
                                       SCALAR> >::const_iterator j, jEnd;
        const_iterator i(begin()), iEnd(end());
        for (; i != iEnd; ++i) {
            const KEY &k = i->key();
            size_t rhdegree = max_depth - basis.degree(k);
            typename std::vector<std::pair<KEY,
                                           SCALAR> >::const_iterator &jEnd = iterators[rhdegree];
    ...
    square_buffered_apply_binary_transform(Vector &result, const sparse_vector &rhs, KeyTransform key_transform) const {
        // create buffer to avoid unnecessary calls to MAP inside loop
        std::vector<std::pair<KEY,
                              SCALAR> > buffer(rhs.map_begin(), rhs.map_end());
        const_iterator i;

        // DEPTH1 == 0
        typename std::vector<std::pair<KEY,
                                       SCALAR> >::const_iterator j;
        for (i = begin(); i != end(); ++i) {
    ...
    template <typename KeyTransform,
            typename IndexTransform>
    void triangular_unbuffered_apply_binary_transform(const sparse_vector &rhs, KeyTransform key_transform,
    ...
    template <typename Vector,
            typename KeyTransform,
            typename IndexTransform>
    void square_buffered_apply_binary_transform(Vector &result, const sparse_vector &rhs, KeyTransform key_transform,
